...
/**
 * @title UpgradeableNFTMarketV2
 * @dev 可升级的 NFT 市场合约 - 第二个版本
 * 增加了离线签名上架 NFT 功能
 */
contract UpgradeableNFTMarketV2 is 
    Initializable,
    ...
    EIP712Upgradeable
{
    ...
    struct Listing {
        ...
        bool isSignatureListing; // 是否为签名上架
    }
    ...
    /**
     * @dev 通过签名创建 NFT listing（V2 新功能）
     * @param nftContract NFT 合约地址
     * @param tokenId NFT 的 token ID
     * @param price 售价（以 wei 为单位）
     * @param seller 卖家地址
     * @param nonce 防重放攻击的 nonce
     * @param deadline 签名过期时间
     * @param signature 卖家的签名
     */
    function createListingWithSignature(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        address seller,
        ...
        // 验证 NFT 所有权和授权
        IERC721 nft = IERC721(nftContract);
        require(nft.ownerOf(tokenId) == seller, "Seller not the owner");
        require(
    ...
    /**
     * @dev 获取 EIP-712 domain separator（用于测试）
     */
